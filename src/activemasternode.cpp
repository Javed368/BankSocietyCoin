...
//
// Bootup the masternode, look for a 10000 Society input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (status != MASTERNODE_REMOTELY_ENABLED)
        {
            ...
            if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
                ...
                /* rewards are not supported in society.conf */
...
{
    CSocietycoinAddress address(collateralAddress);
