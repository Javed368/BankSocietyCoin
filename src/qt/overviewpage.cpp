...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSocietyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSocietyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSocietyAmount = strAnonymizeSocietyAmount.remove(strAnonymizeSocietyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSocietyAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSocietyAmount*COIN) nMaxToAnonymize = nAnonymizeSocietyAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSocietyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(nAnonymizeSocietyAmount));
        //strAnonymizeSocietyAmount = strAnonymizeSocietyAmount.remove(strAnonymizeSocietyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
