...
{
    // Bank Society: check prefix
    if(uri.scheme() != QString("SOCI"))
        return false;
...
{
    // Convert Society:// to Society:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Society://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Society:");
    }
...
void openSocietyConfigfile()
{
    boost::filesystem::path pathDebug = GetDataDir() / "society.conf";

    /* Open society.conf with the associated application */
    if (boost::filesystem::exists(pathDebug))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathDebug)));
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Society.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Society.lnk
...
{
    return GetAutostartDir() / "Society.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Society.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Society\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Society app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Society app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Society-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Society-qt [" + tr("command-line options") + "]                     " + "\n";
