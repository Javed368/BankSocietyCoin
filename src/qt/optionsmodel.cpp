...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSocietyAmount"))
        settings.setValue("nAnonymizeSocietyAmount", 1000);
    nAnonymizeSocietyAmount = settings.value("nAnonymizeSocietyAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSocietyAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSocietyAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSocietyAmount:
            return QVariant(nAnonymizeSocietyAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSocietyAmount:
            nAnonymizeSocietyAmount = value.toInt();
            settings.setValue("nAnonymizeSocietyAmount", nAnonymizeSocietyAmount);
            emit AnonymizeSocietyAmountChanged(nAnonymizeSocietyAmount);
            break;
        default:
            break;
        }
