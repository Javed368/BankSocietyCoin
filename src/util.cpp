...
int nAnonymizeSocietyAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Society
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Society
    // Mac: ~/Library/Application Support/Society
    // Unix: ~/.Society
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Society";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "society.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty society.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "societyd.pid"));
